...
export default function Page() {
  ...
  // Auto-bypass edge gateway connection screen after 2.5s
  useEffect(() => {
  ...
  // Initial Gateway Handshake Screen
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        <div className="w-12 h-12 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin mb-4" />
        <h2 className="text-xl font-bold text-slate-800">Connecting to Edge Gateway...</h2>
        <p className="text-sm text-slate-500 mt-1 mb-6">Handshaking with Vision Terminal</p>
        <button
          onClick={() => setIsLoading(false)}
          className="text-xs bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl font-semibold transition shadow-sm"
        >
          Skip & Open Dashboard
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50/70 text-slate-800 font-sans selection:bg-indigo-500 selection:text-white relative overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-7 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          ...
          <div className="bg-white/90 backdrop-blur-sm p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:shadow-md transition">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs font-medium text-slate-500">currently in store</span>
            </div>
          ...
          <div className="bg-white/90 backdrop-blur-sm p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:shadow-md transition">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs font-medium text-slate-500">{clearedUnits} of {totalCapacity} units</span>
            </div>
        ...
        {/* Detailed Inventory & Live Activity Section */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Main Shelf Stock & Empty Slot Grid (2 Columns) */}
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="text-[11px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100">
                    ...
                    <th className="pb-3 font-semibold text-right">Test Pick</th>
                  </tr>
                </thead>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white p-5 rounded-2xl shadow-sm">
              ...
              <p className="text-xs text-slate-300 leading-relaxed">
                Jab koi customer shelf se item pick karta hai, vision AI use "Invigilated" mark karta hai. Counter par checkout hone ke baad stock status safely update ho jata hai.
              </p>
            </div>
          </div>
        </div>
